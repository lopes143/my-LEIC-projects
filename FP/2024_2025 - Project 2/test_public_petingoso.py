import pytest 
import sys
import Projeto2_privado as fp # <--- Change the name projectoFP to the file name with your project

class TestCustom:
    def test_1(self):
        with pytest.raises(ValueError) as excinfo:
            i1 = fp.cria_posicao('c', 11)        
        assert "cria_posicao: argumentos invalidos" == str(excinfo.value)

    def test_2(self):
        with pytest.raises(ValueError) as excinfo:
            i1 = fp.cria_posicao('z', 1)        
        assert "cria_posicao: argumentos invalidos" == str(excinfo.value)
    ...
    def test_4(self):
        posicao = fp.cria_posicao("a",4)
        assert fp.obtem_pos_col(posicao) == "a"

    def test_5(self):
        posicao = fp.cria_posicao("a",4)
        assert fp.obtem_pos_lin(posicao) == 4

    def test_6(self):
        # depende implementacao
        posicao = (1,1)
        assert fp.eh_posicao(posicao)

    def test_7(self):
        # depende implementacao
        posicao = (-1,1)
        assert not fp.eh_posicao(posicao)
    ...
    def test_13(self):
        # depende implementacao
        posicao = (1,1)
        assert fp.posicao_para_str(posicao) == "a1"

    def test_14(self):
        # depende implementacao
        posicao = (10,10)
        assert fp.posicao_para_str(posicao) == "j10"

    def test_15(self):
        # depende implementacao
        assert fp.str_para_posicao("b2") == (2,2)

    def test_16(self):
        # depende implementacao
        assert fp.str_para_posicao("j10") == (10,10)

    def test_17(self):
        posicao = fp.cria_posicao("b",2)
        assert fp.eh_posicao_valida(posicao,2)

    def test_18(self):
        posicao = fp.cria_posicao("j",2)
        assert not fp.eh_posicao_valida(posicao,2)

    def test_19(self):
        posicao = fp.cria_posicao("j",2)
        assert fp.eh_posicao_valida(posicao,5)

    def test_20(self):
        posicao = fp.cria_posicao("f",3)
        assert fp.eh_posicao_valida(posicao,3)

    def test_21(self):
        posicao = fp.cria_posicao("h",3)
        assert not fp.eh_posicao_valida(posicao,2)
    ...
    def test_23(self):
        i1 = fp.cria_posicao('c', 2)
        assert ('c1', 'd1', 'd2', 'd3', 'c3','b3','b2','b1') == tuple(fp.posicao_para_str(i) for i in fp.obtem_posicoes_adjacentes(i1, 2, True))    

    def test_24(self):
        n = fp.cria_pedra_neutra()
        assert fp.pedra_para_str(n) == ' '

    def test_25(self):
        n = fp.cria_pedra_neutra()
        assert not fp.eh_pedra_preta(n)

    def test_26(self):
        n = fp.cria_pedra_neutra()
        assert not fp.eh_pedra_branca(n)

    def test_26(self):
        p = fp.cria_pedra_preta()
        assert not fp.eh_pedra_branca(p) 

    def test_26(self):
        b = fp.cria_pedra_branca()
        assert not fp.eh_pedra_preta(b) 

    def test_27(self):
        b = fp.cria_pedra_branca()
        assert fp.eh_pedra_branca(b) 

    def test_28(self):
        assert not fp.pedras_iguais(fp.cria_pedra_branca(), fp.cria_pedra_neutra())

    def test_29(self):
        assert fp.pedras_iguais(fp.cria_pedra_neutra(), fp.cria_pedra_neutra())

    def test_29(self):
        assert fp.pedras_iguais(fp.cria_pedra_preta(), fp.cria_pedra_preta())

    def test_30(self):
        assert fp.pedras_iguais(fp.cria_pedra_branca(), fp.cria_pedra_branca())

    def test_31(self):
        n= fp.cria_pedra_neutra()
        assert fp.pedra_para_int(n) == 0

    def test_32(self):
        with pytest.raises(ValueError) as excinfo:
            tab = fp.cria_tabuleiro_vazio("a")
        assert "cria_tabuleiro_vazio: argumento invalido" == str(excinfo.value)

    def test_33(self):
        with pytest.raises(ValueError) as excinfo:
            tab = fp.cria_tabuleiro_vazio(1)
        assert "cria_tabuleiro_vazio: argumento invalido" == str(excinfo.value)
