...
def ordena_posicoes_tabuleiro(tab, tup):
    ...
    output=()
    distEuclQuad=[]
    for i in range(obtem_dimensao(tab)[0]):
        for i2 in range(obtem_dimensao(tab)[1]):
            disToAdd=(i-obtem_coordenadas(tab, posCentral)[0])**2+(i2-obtem_coordenadas(tab, posCentral)[0])**2
            distEuclQuad.append(disToAdd) if disToAdd not in distEuclQuad else None
    
    for i in sorted(distEuclQuad):
        ...
        for i2 in tup:
            ...
            if ((coordCentral[0] - iCoord[0]) ** 2 + (coordCentral[1] - iCoord[1]) ** 2) ** (1 / 2) == i:
...
def marca_posicao(tab, pos, jog):
    ...
    """
    if not (eh_tabuleiro(tab) and eh_posicao(pos) and jog in (1, -1)):
...
def verifica_k_linhas(tab, pos, jog, k):
    ...
    """
    if not (eh_tabuleiro(tab) and eh_posicao(pos) and jog in (1, -1) and type(k)==int):
        raise ValueError("verifica_k_linhas: argumentos invalidos")
    if not (eh_posicao_valida(tab, pos) and k>0):
        raise ValueError("verifica_k_linhas: argumentos invalidos")
...
def eh_fim_jogo(tab, k):
    ...
    """
    if not (eh_tabuleiro(tab) and type(k)==int):
...
def escolhe_posicao_auto(tab, jog, k, lvl):
    ...
    """
    if not (eh_tabuleiro(tab) and jog in (1, -1) and type(k)==int and lvl in ('facil', 'normal', 'dificil')) or eh_fim_jogo(tab, k):
        raise ValueError("escolhe_posicao_auto: argumentos invalidos")
    elif not (k>0 and k<=max(obtem_dimensao(tab)[0], obtem_dimensao(tab)[1], len(obtem_diagonais(tab, 1)[0]))):
        raise ValueError("escolhe_posicao_auto: argumentos invalidos")     
